  ...
  const getBreadcrumbTitle = () => {
    switch (location.pathname) {
      ...
      case '/student/assignments':
        return 'Bài Tập Cần Nộp';
      case '/student/history':
        return 'Lịch Sử & Kết Quả';
      default:
        return 'Không Gian Học Sinh';
    }
  };
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="relative">
          <button
            ...
            className="w-9 h-9 rounded-full text-white font-extrabold text-sm uppercase flex items-center justify-center shadow-md hover:ring-2 hover:ring-emerald-400 transition-all cursor-pointer"
            style={{ backgroundColor: '#10B981' }}
            title={user?.fullName || 'Học sinh'}
            aria-label="User Menu"
          >
            {user?.fullName ? user.fullName.charAt(0) : 'H'}
          </button>
          ...
          {isProfileMenuOpen && (
            ...
            >
              ...
              >
                <p className="font-bold truncate">{user?.fullName || 'Học Sinh Vật Lý'}</p>
                <p className="opacity-60 text-[10px] font-mono truncate">{user?.email || 'student@edulab.vn'}</p>
              </div>
              ...
              <button
                ...
                className="w-full text-left px-3 py-2 hover:bg-slate-500/10 font-semibold cursor-pointer"
              >
                <span>Xem Thư Viện Mô Phỏng</span>
              </button>
