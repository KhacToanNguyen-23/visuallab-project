"""Block local tool calls that directly access sensitive files in Antigravity."""
...
from common import emit, get_tool_call_info, load_config, project_root, read_payload, touched_paths
...
def main() -> None:
    ...
    for path in touched_paths(payload):
        ...
        if matched:
            deny(f"Sensitive path blocked ({matched}): {path.name}")
            return

    tool_name, tool_args = get_tool_call_info(payload)
    command = tool_args.get("CommandLine") or tool_args.get("command") or ""
    if tool_name in {"run_command", "Bash"} and isinstance(command, str):
        matched = command_match(command, allowed)
        if matched:
            deny(f"Command references sensitive data: {matched}")
